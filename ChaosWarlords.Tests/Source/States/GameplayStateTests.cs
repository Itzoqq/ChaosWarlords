...
namespace ChaosWarlords.Tests.States
{
    ...
    public class GameplayStateTests
    {
        ...
        public void Command_BuyCard_BuysCard_WhenAffordable()
        {
            ...
            // Note: We don't check Influence decrease here because that logic is inside 
            // the concrete MarketManager (which we mocked out). We strictly test that the command 
            // delegated the call correctly.
        }
        ...
        public void MoveCardToPlayed_PreservesXPosition_AndPopsUpY()
        {
            ...
            var card1 = new Card("c1", "Left Card", 0, CardAspect.Neutral, 0, 0, 0) { Position = new Vector2(100, 500) };
            var card2 = new Card("c2", "Right Card", 0, CardAspect.Neutral, 0, 0, 0) { Position = new Vector2(250, 500) };
            ...
            // Assert
            Assert.AreEqual(100, card1.Position.X);
            Assert.AreEqual(400, card1.Position.Y);
            Assert.AreEqual(250, card2.Position.X); // Should not move
            Assert.AreEqual(500, card2.Position.Y);
        }
