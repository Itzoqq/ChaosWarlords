...
- New field on `CardEffect` → `Card.CloneEffect`'s object initializer. **Automatically enforced**: `CardTests.Clone_EveryCardEffectProperty_IsPreservedAutomatically` reflects over every `CardEffect` property and fails immediately if a new one isn't copied - you don't need to remember this row or hand-write a new test for it, though a genuinely new PROPERTY TYPE (anything beyond `bool`/`int`/enum/`EffectCondition`) needs one new branch added to that test's `BuildSentinel` helper before it can build a sentinel value for it.
- New `ActionSystem` `Pending*` targeting-state field → `GameStateDto`, `DtoMapper.ToGameStateDto`, `StateRestorer.RestoreState`, `IActionSystem.RestorePendingState`/`ActionSystem.RestorePendingState`, **and** `ActionSystem.ClearState()` (or it survives a return to `Normal` that every other `Pending*` field doesn't - unless that's deliberate, like `PendingDevourCard`). **Automatically enforced, for any field actually named `PendingXxx`**: `StateRestorerTests.EveryPendingProperty_OnIActionSystem_RoundTripsThroughGameStateDtoAndStateRestorer` (the DTO/restore half) and `EveryPendingProperty_ExceptDocumentedExceptions_IsResetByClearState` (the `ClearState()` half) both reflect over `IActionSystem`'s `Pending*` properties automatically - a new one with no matching `RestorePendingState` parameter, or no registered sentinel value in `SetAllPendingPropertiesToSentinels`, fails loudly and tells you exactly what to add. A field that's deliberately NOT cleared by `ClearState()` needs an explicit, commented entry in that test's `PendingPropertiesNotClearedByDesign` set, not a silent skip. This automation relies on the `Pending` name prefix convention holding - a future piece of `ActionSystem` targeting state that serves the same role but isn't named `PendingXxx` would silently fall back to the manual checklist, unprotected by either test.
- New top-level field on `Card` itself (not a `CardEffect`) → `Card.Clone`. **Not yet automated** - still a manual checklist item.
- New field on `Player` → `PlayerDto`, `DtoMapper`'s player mapping (both directions), and `StateRestorer.RestorePlayers`. **Not yet automated** - `Player`'s shape (several properties are private-set and mutated only through differently-named internal methods like `SetPower`/`AddPower`, not a plain public setter) makes a fully generic reflection test meaningfully harder to write correctly than the two above; still a manual checklist item until someone invests in it.
...
The example below is exactly what `Clone_EveryCardEffectProperty_IsPreservedAutomatically` now catches automatically - it's shown for the "why," not as a manual check you still need to run yourself:
...
See `AssassinateCommand.HasSufficientPower`, `ReturnAnySpyCommand.Validate`/`Execute`, `PromoteCommand.HasGenuinePromotionOpportunity`, `PlaceSpyCommand`/`MoveTroopCommand`/`ReturnTroopCommand`/`ReturnOwnSpyCommand`/`SupplantCommand`'s `Validate()`, `ResolveSpyCommand.Validate`/`Execute` (re-derives both the genuine-targeting-state gate and the `EffectType.ReturnEnemySpy` cost waiver, and checks affordability before mutating the board), `DevourCardCommand.Validate` (gates on `CurrentState` being one of the 3 `TargetingDevour*` states), and `DiscardCardCommand.Validate` (gates on `CurrentState == ActionState.TargetingDiscard`) for the real fixes - see `planning.txt`'s open items before treating any command in `Source/Mechanics/Commands/` as a template without checking it against this rule first.
