...
`CardRuleEngine.GetStrategy(EffectType)` is the lookup used throughout `CardEffectProcessor` to resolve the right `IEffectStrategy` (16 implementations in `Mechanics/Rules/Strategies/` as of 2026-09-09, one per `EffectType` family - `EffectTreeSearch.cs`/`SelectOpponentEligibility.cs` in the same folder are shared helpers, not `IEffectStrategy` implementations) for `IsTargetingEffect`/`HasValidTargets`/`SupportsRepeat` checks - this is the extension point for a new effect type's targeting behavior, not a hardcoded switch.
