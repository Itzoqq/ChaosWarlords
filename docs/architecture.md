...
└── Source/
    ...
    └── Mechanics/                           # The "Rules" of the Game (100% MonoGame-free)
        ...
        └── Rules/                           # Pure Logic Engines
            ...
            ├── CardEffectProcessor.cs       # Effect-tree construction/repeat-expansion pipeline (ResolveEffects) - decides WHICH node to apply and WHEN
...
**Newer immediate-targeting primitives.** Several `EffectType`/`IEffectStrategy`/`ActionState` triples were added from 2026-09 onward, following the same established shape as `Assassinate`/`Supplant`/`PlaceSpy` (a new `EffectType` + `Strategies/` implementation + `ActionState`, resolved via the execution stack):
- `EffectType.SelectOpponent` / `SelectOpponentStrategy` / `ActionState.TargetingOpponentSelect` - the generic "target a player" primitive: the active player picks which opponent (matching an eligibility threshold) becomes `TurnManager.ForcedActingPlayer` for whatever `OnSuccess` chains off it, e.g. Cranium Rats' "choose one opponent with more than 3 cards to discard a card". Eligibility is pluggable (`SelectOpponentEligibility`, shared by the Strategy's own lookahead and `SelectOpponentCommand.Validate()` so the two can't diverge): the default is a hand-size threshold, or `CardEffect.RequiresAdjacencyToRecentDeploys` switches it to "has a troop adjacent to a just-deployed node" (Gibbering Mouther).
...
- `EffectType.PromoteSelf`, `EffectType.ReturnUnitOrSpy`, `EffectType.DeployFromTrophyHall`, `EffectType.ReturnEnemySpy` - later additions in the same shape (unconditional self-promotion; a troop-or-spy target-type union; deploying a troop funded by a trophy hall's captured-troop composition instead of the normal barracks; returning an enemy spy through the same machinery the paid basic action uses). See `reference/patterns.md` in the `tyrants-rules` skill for the design rationale behind each.
- `EffectType.PromoteTopOfDeck` - non-targeting, no player choice at all (unlike `PromoteFromPile`): promotes whatever card currently sits on top of the active player's own deck (Hezrou, Nalfeshnee), via `Player.TryPromoteTopOfDeck`/`IPlayerStateManager.TryPromoteTopOfDeck` - reuses `Deck.Draw(1, random)`'s reshuffle-on-empty behavior rather than a dedicated "peek" method, redirecting the drawn card straight to the Inner Circle instead of Hand.
- `EffectType.DeployTroop` / `DeployTroopStrategy` / `ActionState.TargetingDeployTroop` / `ActionSystem.PendingDeployedNodes` - an IMMEDIATE, node-tracked Deploy (Gibbering Mouther: "Deploy 2 troops, then choose an opponent with a troop adjacent to at least 1 of them"), distinct from every other "Deploy N troops" card's `GainResource(Troops)` -> `PendingFreeTroops` deferred-credit shape, which can't support a "then act on where they landed" follow-up at all since it never tracks which nodes the credit was actually spent on.
- `EffectType.ForceRecruit` - gives a specific card (`CardEffect.TargetCardId`, e.g. `insane_outcast`) directly to a player, bypassing the market row via `ICardDatabase.GetCardById` + `IPlayerStateManager.AcquireCard`. Two modes: the default resolves against whoever an earlier `EffectType.SelectOpponent` step chose (Gibbering Mouther); `CardEffect.AppliesToEachOpponent` instead loops every opponent of the real card-playing player via `ITurnManager.GetOpponentsInSeatOrder` (Ghoul/Demogorgon) - no `SelectOpponent` step needed since giving a specific card requires no player choice.
...
- `IEffectStrategy.SupportsRepeat` + `EffectContext.RemainingRepeats` - lets a targeting effect ask to be resolved N times in a row instead of once. Several strategies opt in as of 2026-09 (`AssassinateStrategy`/Deathblade, `MoveUnitStrategy`/Council Member, `PlaceSpyStrategy`/Masters of Sorcere, `SupplantStrategy`/Demogorgon, `DeployTroopStrategy`/Gibbering Mouther, `ReturnUnitOrSpyStrategy`/Intellect Devourer) - check `IEffectStrategy` implementations directly for the current list rather than trusting this enumeration to stay in sync as more cards ship. `RemainingRepeats` is part of `EffectContextDto` and round-trips through the same snapshot/restore machinery as the rest of the execution stack (see Key Systems #6).
- `CardEffect.DynamicAmountSource` / `DynamicAmountDivisor` - computes an effect's numeric amount from live board state at resolution time (via `DynamicAmountResolver.ResolveAmount`) instead of a fixed JSON constant, e.g. White Dragon.
- `Card.ReactiveDiscardEffect` - a top-level card field (not a `CardEffect`) applied by `DiscardCardCommand.Execute` when an opponent forces this card to be discarded from hand, e.g. Grimlock (`DrawCard`) or Umber Hulk (`EffectType.ForceCausingOpponentDiscard` - forces the SPECIFIC opponent who caused the discard, resolved as `TurnManager.CurrentTurnContext.ActivePlayer`, to discard a card of their own in return). The latter reuses `MatchManager`'s existing Neogi-style `_pendingDiscardQueue`/`AdvanceOpponentDiscard` machinery (`MatchManager.EnqueueReactiveDiscard`) rather than a new queue, gated by a `_discardPhaseEndsTurn` flag so a mid-turn reactive phase doesn't also end the turn the way Neogi's end-of-turn one does - started via `MatchManager.ResumeReactiveDiscardQueue`, called from `ActionSystem.ReleaseForcedActingPlayerIfOwnedByExecutionStack` (the one point already positioned after `ClearState()` has reset `CurrentState` back to `Normal`, so a fresh `StartTargeting` call there actually sticks instead of being immediately overwritten), since `ReactiveDiscardEffect` resolves from inside the interrupting chain's own resolution and can't safely start a new targeting sequence synchronously at enqueue time.
  `EffectType.PromoteInsteadOfDiscard` (Ambassador) is the first `ReactiveDiscardEffect` shape that REPLACES the discard rather than adding to it, and the first that's a genuine player choice: `DiscardCardCommand` special-cases the type directly (`DiscardCardCommand.PromoteInsteadOfDiscard`, validated against `card.ReactiveDiscardEffect?.Type` AND `TurnManager.ForcedActingPlayer == the discarding player`) rather than going through `CardEffectApplier`'s `_effectHandlers` dispatch table - there's no `EffectContext` for a bare `ApplyEffect` call to hang a further choice off of. The player's choice itself is surfaced through `IGameplayState.RequestOptionalEffect` (a thin pass-through to the existing generic `IUIEventMediator.RequestOptionalEffect` popup `ActionSystem.OnInteractionRequested` itself resolves through), invoked directly from `DiscardInputMode.HandleInteraction` rather than the ExecutionStack - the resulting accept/decline closures dispatch two different `DiscardCardCommand` instances (`PromoteInsteadOfDiscard: true`/`false`) through the normal `RecordAndExecuteCommand` path, so the choice is fully part of the recorded/replayed command rather than a client-only branch.
- `CardEffect.TargetsAffectedPlayer` - "outcome-dependent targeting": a chained `OnSuccess`/`Alternative` effect acts on whoever the immediately preceding `Assassinate`/`Supplant` step actually hit (`ActionSystem.PendingAffectedPlayerColor`, set by `PerformAssassinate`/`PerformSupplant` right before the map mutation, mirroring `PendingSite`'s own pattern), not the card's owner - e.g. Mindwitness ("Assassinate a troop. If that troop belonged to another player and they have more than 3 cards, they must discard a card."). `CardEffectProcessor.PushEffectContext`'s `TryResolveActor` resolves the actor and gates on `Condition`/`HasValidTargets` against it *before* calling `TurnManager.BeginForcedActingPlayer` - the same `ForcedActingPlayer` machinery `SelectOpponent` established, just reached automatically instead of via a player choice. This also closed a previously-latent gap: a targeting effect's own `Condition` was never checked before being pushed (only automatic effects were gated, via `ApplyEffect`) - now checked uniformly for every effect.
- `CardEffect.RequiredPromotionAspect` - restricts an `EffectType.Promote` credit (the pre-existing deferred end-of-turn "promote a card played this turn" flow - `TurnContext.AddPromotionCredit`/`PromoteInputMode`, e.g. `Wyrmspeaker`'s plain mandatory shape, Cultist of Myrkul's "up to N") to only be redeemable against a card whose `Aspect` matches, e.g. Air/Fire/Water Elemental Myrmidon's "promote an Obedience [`CardAspect.Order`] card played this turn." Threaded onto `TurnContext`'s private `PromotionCredit` record (`RequiredAspect`); `HasValidCreditFor`/`ConsumeCreditFor` both route through a shared `CreditAllows` check (self-exclusion AND, if the credit has a filter, an aspect match) so the two can never diverge. Null (the default) means no filter, so `Wyrmspeaker`/Cultist of Myrkul/Zuggtmoy/Blue Dragon's existing unfiltered credits are unaffected - Earth Elemental Myrmidon's own "promote ANOTHER card played this turn" (no aspect wording at all) uses this same unfiltered `EffectType.Promote` shape too, not a new mechanism. Ambassador's own primary ability ("At end of turn, promote another card played this turn") is the identical unfiltered shape; its separate *optional discard-replacement* ability is `EffectType.PromoteInsteadOfDiscard` (see the `Card.ReactiveDiscardEffect` bullet above). A filtered credit also introduces a real soft-lock risk an unfiltered one structurally can't (self-exclusion alone always admits a full derangement across 2+ distinct-source mandatory credits; an aspect filter can exclude an arbitrary number of cards at once, so 2+ filtered mandatory credits sharing a scarce matching pool can strand a sibling credit with zero legal targets, which - being mandatory - can neither be declined nor clicked past): `TurnContext.ForfeitUnsatisfiableCredits(playedCards)` drops any credit no currently-played card can satisfy, called both by `UIEventMediator.HandleEndTurnWithPromotionCheck` (before a redemption prompt opens at all - a credit can be dead on arrival) and by `PromoteInputMode.HandleLeftClick` (after each single credit is consumed, since consuming one can strand another). `ConsumeCreditFor` also now prefers consuming a filtered credit over an unfiltered one when a target satisfies both, reducing (without a hard matching-theoretic guarantee of eliminating) how often click order alone causes an avoidable forfeiture.
- `Card.CreatureType` (`CardCreatureType` enum) - a genuinely separate data dimension from `Aspect`: the creature-type label printed in a card's top-right corner (Human, Undead, Aberration, Monstrosity, Illithid, ...). Only `CardCreatureType.Undead` is modeled (High Priest of Myrkul's "promote ANY NUMBER of Undead cards played this turn" is the only shipped mechanic that reads it) - every other card defaults to `None`, which does NOT assert "this card has no real type," only "nothing needs to filter on it yet." Extend the enum (and retroactively tag the newly-relevant already-shipped cards in `cards.json`) only once another mechanic actually needs a different type filter.
- `CardEffect.RequiredPromotionCreatureType` - a sibling filter to `RequiredPromotionAspect`, threaded onto the same `PromotionCredit` record and `CreditAllows` check, but matching `Card.CreatureType` instead of `Aspect`.
- `CardEffect.PromoteAnyNumber` - "promote ANY NUMBER of [filtered] cards played this turn" (as opposed to a fixed `Amount`, e.g. `Wyrmspeaker`'s 1 or Cultist of Myrkul's "up to 2"). `CardEffectApplier.ApplyPromote` routes to `TurnContext.AddUnboundedPromotionCredit(source, requiredCreatureType)` instead of the fixed-amount `AddPromotionCredit` - this only registers a pending descriptor (`(Card Source, CardCreatureType? RequiredCreatureType)`, null meaning no filter at all, same convention as `PromotionCredit`'s own nullable filters), since the real count isn't known until the turn's played-card set is final. `TurnContext.ExpandPendingUnboundedCredits` (private) materializes each pending descriptor into exactly one discrete, always-optional `PromotionCredit` per currently-eligible played card (self-exclusion still applies) - called from `HasValidCreditFor`/`ConsumeCreditFor`/`ForfeitUnsatisfiableCredits`/`CanDeclineRemainingPromotions`, all of which are only ever reached in production once a real redemption session has already started. **Deliberately NOT called from `PendingPromotionsCount`'s own getter** (see that property's own doc comment) - a future caller reading it speculatively, before a real redemption session has forced expansion via one of the 4 methods above, must not assume it reflects an unbounded credit yet. Every genuine redemption entry point (`UIEventMediator.HandleEndTurnWithPromotionCheck`) always calls `ForfeitUnsatisfiableCredits` before trusting `PendingPromotionsCount`, which is what makes the count accurate by the time anything real reads it. 2+ sources sharing the same eligible pool (e.g. two High Priests) over-credit relative to the real number of distinct targets - harmless, since the excess becomes unsatisfiable the moment the real targets are gone and gets swept by `ForfeitUnsatisfiableCredits` like any other stranded optional credit.
...
`ConditionType` covers the usual resource/board-state checks (`ControlsSite`, `HasTroopsDeployed`, `HasResourceAmount`, `InnerCircleCount`, `HandSize`, `TrophyHallCount` - any-color troop count, `PlayerTrophyHallCount` - same sum but excluding Neutral/white troops, e.g. Dragonclaw) plus `ConditionType.OpponentPresentAtSite`: reads `ActionSystem.PendingSite` to check whether another player has a spy or troop (`SitePresenceType.Spy`/`SitePresenceType.Troop`) at the site a chained `PlaceSpy` effect just targeted, e.g. Banshee (bonus Power if an opponent has a spy there) and Infiltrator (bonus Power if an opponent has a troop there).

`CardRuleEngine.GetStrategy(EffectType)` is the lookup used throughout `CardEffectProcessor`/`CardEffectApplier` to resolve the right `IEffectStrategy` (16 implementations in `Mechanics/Rules/Strategies/` as of 2026-09-09, one per `EffectType` family - `EffectTreeSearch.cs`/`SelectOpponentEligibility.cs` in the same folder are shared helpers, not `IEffectStrategy` implementations) for `IsTargetingEffect`/`HasValidTargets`/`SupportsRepeat` checks - this is the extension point for a new effect type's targeting behavior, not a hardcoded switch.
...
- `CardEffectApplier.ApplyDevour` (moved from `CardEffectProcessor` 2026-09-10): Reduced from CC 16 to 6 (63% reduction)
