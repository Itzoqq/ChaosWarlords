...
namespace ChaosWarlords.Source.Managers
{
    public class ActionSystem : IActionSystem
    {
        ...
        public void SetPendingAffectedPlayerColor(PlayerColor? color)
        {
            PendingAffectedPlayerColor = color;
        }
        ...
        public void AddPendingDeployedNode(MapNode node)
        {
        ...
        private readonly MapActionSubsystem _mapActionSubsystem;
        ...
        public ActionSystem(ITurnManager turnManager, IMapManager mapManager, IGameLogger logger, IPlayerStateManager playerStateManager, IMarketManager marketManager)
        {
            ...
            // Basic map-action execution (Assassinate/Supplant/ReturnTroop/DeployTroop/
            // DeployFromTrophyHall/MoveTroop) lives in its own class too, extracted 2026-09-10
            // the same way Devour/Spy already were - see MapActionSubsystem's doc comment and
            // planning.txt TIER 2 item 12.
            _mapActionSubsystem = new MapActionSubsystem(_mapManager, _turnManager, _logger, _playerStateManager, this, _devourSubsystem);
        ...
        public void SetMatchManager(IMatchManager matchManager)
        {
            ...
            _mapActionSubsystem.SetMatchManager(matchManager);
        }
        ...
        // Basic map-action execution lives in MapActionSubsystem - see its doc comment and
        // planning.txt TIER 2 item 12. These stay on ActionSystem/IActionSystem as thin
        // delegations, same as PerformPlaceSpy/PerformSpyReturn's existing pattern below,
        // so no external caller (commands, tests) needed to change at all.

        public void PerformAssassinate(MapNode node, string? cardId, string? devourCardId = null)
        {
            _mapActionSubsystem.PerformAssassinate(node, cardId, devourCardId);
        }

        public void PerformReturnTroop(MapNode node, string? cardId)
        {
            _mapActionSubsystem.PerformReturnTroop(node, cardId);
        }

        public void PerformSupplant(MapNode node, string? cardId, string? devourCardId = null)
        {
            _mapActionSubsystem.PerformSupplant(node, cardId, devourCardId);
        }

        public void PerformDeployFromTrophyHall(MapNode node, PlayerColor sourcePlayerColor, PlayerColor troopColor, string? cardId)
        {
            _mapActionSubsystem.PerformDeployFromTrophyHall(node, sourcePlayerColor, troopColor, cardId);
        }

        public void PerformDeployTroop(MapNode node, string? cardId)
        {
            _mapActionSubsystem.PerformDeployTroop(node, cardId);
        }
        ...
        {
            _mapActionSubsystem.PerformMoveTroop(source, dest, cardId);
        }
