using Microsoft.Xna.Framework.Content;
using Microsoft.Xna.Framework.Graphics;
using System.Collections.Generic;
using ChaosWarlords.Source.Contexts;
using ChaosWarlords.Source.Systems;
using ChaosWarlords.Source.Views;

namespace ChaosWarlords.Source.Interfaces
{
    public interface IGameplayView
    {
        int HandY { get; }
        int PlayedY { get; }

        List<CardViewModel> HandViewModels { get; }
        List<CardViewModel> PlayedViewModels { get; }
        List<CardViewModel> MarketViewModels { get; }

        void LoadContent(ContentManager content);
        void Update(MatchContext context, InputManager inputManager, bool isMarketOpen);
        void Draw(SpriteBatch spriteBatch, MatchContext context, InputManager inputManager, UIManager uiManager, bool isMarketOpen, string targetingText, bool isPopupOpen, bool isPauseMenuOpen);
        void DrawSetupPhaseOverlay(SpriteBatch spriteBatch, Entities.Player activePlayer);
    }
}
